...
public class Program {
    ...
    public static void main(String[] args) {
        ...
        while (f){
            ...
            System.out.println("8. Удалить все текстуры модели");
            System.out.println("9. Копировать модель");
                    ...
                    switch (no) {
                        ...
                        case 8:
                            ...
                            if (scanner.hasNextInt()){
                                int modelNo = scanner.nextInt();
                                scanner.nextLine();
                                editor3D.editModel(modelNo, (model) -> {
...
class Editor3D implements UILayer{
    ...
    @Override
    public void editModel(int modelId, Function<Model3D,Model3D> action) {
        Model3D model3D = findModel((ArrayList<Model3D>)businessLogicalLayer.getAllModels(), modelId);
        databaseAccess.removeEntity(model3D);
        databaseAccess.addEntity(action.apply(model3D));
    }

    @Override
    public void copyModel(int modelId) {
        Model3D model3D = findModel((ArrayList<Model3D>)businessLogicalLayer.getAllModels(), modelId);
        databaseAccess.addEntity(new Model3D(model3D));
    }

    private Model3D findModel(List<Model3D> model3DS, int modelId)  {
        return model3DS.stream().filter(model -> model.getId()==modelId).findFirst().orElseThrow(() -> new RuntimeException("Номер модели указан некорректною."));
    }
}
...
interface UILayer{
    ...
    void editModel(int modelId, Function<Model3D,Model3D> action);
...
class Model3D implements Entity{
    ...
    public Model3D(Model3D model3D) {
        this.textures = model3D.getTextures();
    }
